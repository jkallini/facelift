...
def eager_attention_forward(module, query, key, value, attention_mask, head_mask=None, **kwargs):
    ### NEW CODE ###
    if getattr(module.config, "rope", False):
        query, key = module.rope(query, key)
    ### NEW CODE ###
    ...
    if getattr(module.config, "alibi", False):
        ...
        if module.constant_alibi_slope:
            alibi_bias = get_relative_positions(seq_len).unsqueeze(0)
        else:
    ...
    if getattr(module.config, "geometric_attention", False):
    ...
    def __init__(self, config):
        ...
        self.has_absolute_positional_encodings = \
            config.has_positional_encodings and \
                not (getattr(config, "geometric_attention", False)
                     or getattr(config, "alibi", False)
...
class GPT2ForSequenceClassification(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            #### NEW CODE ####
            # If pad token is same as eos token, use first pad token index
            eos_offset = -1 if self.config.pad_token_id == self.config.eos_token_id else 0
            last_non_pad_token = (token_indices * non_pad_mask).argmax(-1) + eos_offset
            #### NEW CODE ####
        else:
            last_non_pad_token = -1
            logger.warning_once(
